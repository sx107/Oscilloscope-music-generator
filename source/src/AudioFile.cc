...
bool AudioFile<T>::saveToWaveFile (std::string filePath)
{
    ...
    if(bitDepth == 32) {addInt16ToFileData (fileData, 3);} // audio format = 1
	else {addInt16ToFileData (fileData, 1);}
    ...
    for (int i = 0; i < getNumSamplesPerChannel(); i++)
    {
        for (int channel = 0; channel < getNumChannels(); channel++)
        {
            ...
            else if (bitDepth == 24)
            {
				...
				float s = std::max(-1.0d, std::min(double(samples[channel][i]), 1.0d));
